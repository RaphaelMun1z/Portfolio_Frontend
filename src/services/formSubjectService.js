...
// Get form subjects by id
const getFormSubjectsByFormType = async (type) => {
    const config = requestConfig("GET", null)

    try {
        const res = await fetch(api + "/form-subjects/form-type/" + type, config)
            .then((res) => res.json())
            .catch((err) => err)
